...
- **Monitoring endpoints** (`/healthz` `/readyz` `/poolz` `/metrics` `/v1/status` `/v1/version`) are unauthenticated and **must bind loopback-only**. The core's REST monitor already defaults to `127.0.0.1:8081` (`FILEENGINE_HTTP_METRICS_ADDR`), and nothing widens it: `core.conf` does not set it, the compose stack publishes no port for it, and the Ansible role leaves the default. An earlier note here described it as defaulting to `0.0.0.0` and being a known exposure — that has not been true since the L2 security review, and the claim was misleading in the dangerous direction, sending operators to verify an exposure that does not exist.
- **The gRPC port likewise binds `127.0.0.1` by default** (`FILEENGINE_GRPC_HOST`). It is the trusted-access path — the core authorizes the identity a caller presents rather than authenticating it — so it must only ever be reachable by the trusted world-facing services. Containers and multi-host deployments widen it to `0.0.0.0` explicitly, where the container network supplies the isolation and the port is never published to the host.
