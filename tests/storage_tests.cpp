...
void test_storage_path_generation() {
    ...
    fileengine::Storage storage(test_path, false, false);
    ...
    // Test SHA256 desaturated path generation
    std::string desaturated = storage.get_sha256_desaturated_path(uid);
    assert(!desaturated.empty());  // Should return a non-empty path
...
void test_sha256_desaturation() {
    std::cout << "Testing SHA256 desaturation functionality..." << std::endl;
    
    std::string test_path = "/tmp/fileengine_desat_test_" + fileengine::Utils::generate_uuid();
    std::filesystem::create_directory(test_path);
    
    fileengine::Storage storage(test_path, false, false);
    
    // Test with different UUIDs to make sure they create different paths
    std::string uid1 = "test-uid-123-abc-def";
    std::string uid2 = "test-uid-456-ghi-jkl";
    
    std::string path1 = storage.get_sha256_desaturated_path(uid1);
    std::string path2 = storage.get_sha256_desaturated_path(uid2);
    
    // Both paths should be non-empty
    assert(!path1.empty());
    assert(!path2.empty());
    
    // Paths should be different for different UIDs
    assert(path1 != path2);
    
    // Both should have the expected structure (first few chars of hash)
    assert(path1.length() >= 2);  // Should have at least 2 characters
    assert(path2.length() >= 2);  // Should have at least 2 characters
