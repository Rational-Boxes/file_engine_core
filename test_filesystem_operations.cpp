...
#include "core/include/fileengine/acl_manager.h"

int main() {
    std::cout << "Testing filesystem operations with detailed logging..." << std::endl;
    ...
    // Test creating a directory with an empty parent UID (should work for root user)
    auto result = filesystem->mkdir("", "direct_test_dir", "root", 0755, "");
    
    if (result.success) {
        std::cout << "SUCCESS: Created directory in root with UID: " << result.value << std::endl;
        
        // Now test creating a file in the root directory
        std::cout << "Testing touch with empty parent UID (root file creation)..." << std::endl;
        auto file_result = filesystem->touch("", "direct_test_file.txt", "root", "");
        if (file_result.success) {
            std::cout << "SUCCESS: Created file in root directory with UID: " << file_result.value << std::endl;
        } else {
            std::cout << "FAILED: Could not create file in root: " << file_result.error << std::endl;
        }
        
        // Test creating a file in the directory we just created
        std::cout << "Testing touch with directory parent UID..." << std::endl;
        auto dir_file_result = filesystem->touch(result.value, "test_file_in_dir.txt", "root", "");
        if (dir_file_result.success) {
            std::cout << "SUCCESS: Created file in directory with UID: " << dir_file_result.value << std::endl;
        } else {
            std::cout << "FAILED: Could not create file in directory: " << dir_file_result.error << std::endl;
        }
    } else {
        std::cout << "FAILED: Could not create directory in root: " << result.error << std::endl;
        return 1;
    }
